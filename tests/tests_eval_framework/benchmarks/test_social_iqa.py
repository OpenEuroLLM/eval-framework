...
from eval_framework.benchmarks.social_iqa import social_iqa_mc_olmes
from eval_framework.tasks.registry import Registry
from eval_framework.tasks.task_names import register_social_iqa_tasks
...
# Registry for this test suite only holding the composed social_iqa task.
_social_iqa_registry = Registry()
register_social_iqa_tasks(registry=_social_iqa_registry)


@pytest.mark.formatter_hash
@pytest.mark.parametrize("formatter_cls", [Llama3Formatter, ConcatFormatter, NoStripConcatFormatter])
@pytest.mark.parametrize("task_name", _social_iqa_registry.task_names())
def test_formatter_hash(task_name: str, formatter_cls: type[BaseFormatter]) -> None:
    run_formatter_hash_test(task_name, formatter_cls, registry=_social_iqa_registry)
